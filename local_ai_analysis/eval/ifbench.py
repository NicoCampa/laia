...
class IFBenchRunner:
    ...
    def run(
        ...
        completed_samples = 0
        resume_records = (
            load_resume_records(samples_path, key=_ifbench_record_key)
        ...
        with samples_path.open(sample_file_mode(resume_records), encoding="utf-8") as sample_file:
            for index, row in enumerate(rows, start=1):
                ...
                row_key = _ifbench_sample_key(self.settings, row, prompt)
                ...
                if cached_record and record_matches(
                    ...
                    strict_output = _output_from_record(cached_record, "strict")
                    loose_output = _output_from_record(cached_record, "loose")
                ...
                resume_announced = maybe_announce_resume(
                    ...
                    total_samples=total_samples,
                    correct_samples=loose_counts.prompt_correct,
...
def _ifbench_record_key(record: dict[str, Any]) -> str | None:
    return cache_key(
        record.get("dataset"),
...
def _output_from_record(record: dict[str, Any], prefix: str) -> Any:
    return SimpleNamespace(
        instruction_id_list=list(record.get("instruction_id_list") or []),
        follow_instruction_list=list(
            record.get(f"{prefix}_follow_instruction_list") or []
        ),
        follow_all_instructions=bool(record.get(f"{prefix}_follow_all")),
    )
